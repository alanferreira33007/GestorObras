...
# CSS OTIMIZADO (SEM @import EXTERNO PARA CARREGAR MAIS RÁPIDO)
st.markdown("""
<style>
    html, body, [class*="css"] { 
...
# ==============================================================================
# 3. MOTOR PDF (ENTERPRISE V5) - LAZY IMPORT
# ==============================================================================
def gerar_pdf_empresarial(escopo, periodo, vgv, custos, lucro, roi, df_cat, df_lanc):
    ...
    total_lbl = Paragraph("<b>TOTAL ACUMULADO GASTO (ATÉ EMISSÃO)</b>", ParagraphStyle('TLabel', parent=styles['Normal'], fontSize=10, alignment=TA_RIGHT))
    ...
    t_total = Table([[total_lbl, total_val]], colWidths=[12*cm, 5*cm])
...
@st.cache_resource
def get_conn():
    # LAZY IMPORT (reduz o custo de inicialização do app)
    import gspread
...
def password_entered():
    if st.session_state.get("password_input") == st.secrets["password"]:
        st.session_state.auth = True
        st.session_state.pop("login_error", None)
        # marca para exibir shell e carregar dados depois
...
def logout():
    ...
    st.session_state.pop("data_obras", None)
    st.session_state.pop("data_fin", None)
...
# ==============================================================================
# 6. SIDEBAR PRIMEIRO (RENDER INSTANTÂNEO)
# ==============================================================================
with st.sidebar:
    st.markdown("""
...
# ==============================================================================
# 7. SHELL + CARREGAMENTO (EVITA TELA EM BRANCO APÓS LOGIN)
# ==============================================================================
# Renderiza algo imediatamente, antes de buscar Google Sheets
shell = st.container()
with shell:
    st.markdown("")

# Carrega dados somente depois do sidebar/shell aparecerem
if "data_obras" not in st.session_state or "data_fin" not in st.session_state:
    info_box = st.empty()
    info_box.info("🔄 Carregando dados do banco (Google Sheets)...", icon="⏳")
    with st.spinner("Sincronizando..."):
        try:
            ...
            st.session_state["data_obras"] = pd.DataFrame(columns=OBRAS_COLS)
            st.session_state["data_fin"] = pd.DataFrame(columns=FIN_COLS)
    info_box.empty()
else:
    df_obras = st.session_state["data_obras"]
    df_fin = st.session_state["data_fin"]

df_obras = st.session_state.get("data_obras", pd.DataFrame(columns=OBRAS_COLS))
df_fin = st.session_state.get("data_fin", pd.DataFrame(columns=FIN_COLS))

lista_obras = df_obras["Cliente"].unique().tolist() if not df_obras.empty and "Cliente" in df_obras.columns else []
...
if sel == "Dashboard":
    ...
    with c_sel:
        if lista_obras:
            ...
            st.warning("Cadastre uma obra para começar.")
            st.stop()
    ...
    if escopo == "Visão Geral (Todas as Obras)":
        vgv = df_obras["Valor Total"].sum() if "Valor Total" in df_obras.columns else 0.0
    ...
    custos = df_show["Valor"].sum() if not df_show.empty else 0.0
    lucro = vgv - custos
    roi = (lucro / custos * 100) if custos > 0 else 0.0
    perc = (custos / vgv) if vgv > 0 else 0.0
    ...
    with g1:
        ...
        if not df_show.empty:
            ...
            fig = px.area(df_ev, x="Data_DT", y="Acumulado")
    ...
    if not df_show.empty:
        ...
        dmin = df_show["Data_DT"].min()
        dmax = df_show["Data_DT"].max()
        if pd.isna(dmin) or pd.isna(dmax):
            per_str = "Período não identificado"
        else:
            per_str = f"De {dmin.strftime('%d/%m/%Y')} até {dmax.strftime('%d/%m/%Y')}"
        ...
        st.download_button(
            label=label_btn_pdf,
            data=pdf_data,
            file_name=f"Relatorio_{str(escopo).replace(' ', '_')}_{date.today()}.pdf",
...
# --- OBRAS ---
    ...
    with st.expander("➕ Novo Cadastro (Clique para expandir)", expanded=False):
        with st.form("f_obra_completa", clear_on_submit=False):
            ...
            with c7:
                status = st.selectbox("Fase Atual", ["Projeto", "Fundação", "Alvenaria", "Acabamento", "Concluída", "Vendida"],
                                      key="k_ob_status")
            ...
            if submitted:
                ...
                if erros:
                    ...
                    try:
                        ...
                        ids_existentes = pd.to_numeric(df_obras["ID"], errors="coerce").fillna(0) if "ID" in df_obras.columns else pd.Series([0])
                        novo_id = int(ids_existentes.max()) + 1
                        ws.append_row([novo_id, nome_obra.strip(), endereco.strip(), status, float(valor_venda),
                                       data_inicio.strftime("%Y-%m-%d"), prazo_entrega.strip(), float(area_const),
                                       float(area_terr), int(quartos), float(custo_previsto)])
    ...
    if not df_obras.empty:
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Status": st.column_config.SelectboxColumn("Fase",
                    options=["Projeto", "Fundação", "Alvenaria", "Acabamento", "Concluída", "Vendida"], required=True, width="medium"),
                            ...
                            try:
                                ...
                                with st.spinner("Salvando..."):
                                    for _, row in edited_df.iterrows():
                                        id_obra = row["ID"]
                                        found_cell = ws.find(str(int(id_obra)), in_column=1)
                                        if found_cell:
                                            original_row = df_obras[df_obras["ID"] == id_obra].iloc[0]
                                            update_values = []
                                            for col in OBRAS_COLS:
                                                val = row[col] if col in row else original_row.get(col, "")
